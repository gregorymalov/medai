...
class CallSubcriteria(BaseModel):
    ...
    patient_name: Optional[str] = Field(None, description="Выяснение имени пациента (✅, !, ±)")
    need_identification: Optional[str] = Field(None, description="Выявление потребностей (✅, !, ±)")
    clinic_presentation: Optional[str] = Field(None, description="Презентация клиники (✅, !, ±)")
    service_presentation: Optional[str] = Field(None, description="Презентация услуг (✅, !, ±)")
    doctor_presentation: Optional[str] = Field(None, description="Презентация врачей (✅, !, ±)")
    ...
    passport: Optional[str] = Field(None, description="Напоминание о паспорте (✅, !, ±)")
    objection_handling: Optional[str] = Field(None, description="Работа с возражениями (✅, !, ±)")
    next_step: Optional[str] = Field(None, description="Обсуждение следующего шага (✅, !, ±)")
    speech_quality: Optional[str] = Field(None, description="Качество речи (✅, !, ±)")
    initiative: Optional[str] = Field(None, description="Проявление инициативы (✅, !, ±)")
    recall_appeal: Optional[str] = Field(None, description="Апелляция к предыдущему звонку (✅, !, ±)")
    clarification: Optional[str] = Field(None, description="Уточнение вопроса (✅, !, ±)")
...
class CallMetrics(BaseModel):
    ...
    customer_satisfaction: str = Field(..., description="Удовлетворенность клиента (high/medium/low)")
    overall_score: float = Field(..., description="Общая оценка (0-10)")
    fg_percent: Optional[float] = Field(None, description="Процент выполнения критериев (FG%)")
    subcriteria: Optional[CallSubcriteria] = Field(None, description="Детальные оценки по подкритериям")
...
class CallMetricsRecord(BaseModel):
    ...
    contact_id: Optional[int] = Field(None, description="ID контакта AmoCRM")
    lead_id: Optional[int] = Field(None, description="ID сделки AmoCRM")
    call_type: Optional[str] = Field(None, description="Тип звонка (входящий/исходящий)")
    call_category: Optional[str] = Field(None, description="Категория звонка (первичка_1/первичка_перезвон/подтверждение/вторичка)")
    traffic_source: Optional[str] = Field(None, description="Источник трафика")
    client_request: Optional[str] = Field(None, description="Запрос клиента (краткое описание)")
    conversion: Optional[bool] = Field(None, description="Конверсия (True - да, False - нет)")
    metrics: CallMetrics = Field(..., description="Метрики звонка")
    comments: Optional[str] = Field(None, description="Комментарии к оценке")
    recommendations: Optional[List[str]] = Field(None, description="Рекомендации по улучшению")
    call_classification: int = Field(..., description="Классификация звонка (1-8)")
    crm_link: Optional[str] = Field(None, description="Ссылка на карточку в CRM")
    transcription_link: Optional[str] = Field(None, description="Ссылка на транскрибацию")
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="Дата создания записи")
